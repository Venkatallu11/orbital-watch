...
from orbital_watch.discover import fetch_group


def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Discover real satellites by CelesTrak GROUP.")
    parser.add_argument("--groups", required=True, help="Comma-separated CelesTrak GROUP names")
    ...
    for group in args.groups.split(","):
        ...
        results[group] = [{"norad_id": e.norad_id, "name": e.name} for e in sample]
        print(f"\n=== {group} ({len(entries)} total, showing {len(sample)}) ===")
        for e in sample:
